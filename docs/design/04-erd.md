...
- **coupon_templates**: 쿠폰 발급 기준 정의. 수량 제한 없음 — 같은 쿠폰을 여러 회원이 발급받을 수 있으며 중복 발급(동일 회원+동일 템플릿)만 DB UNIQUE로 방지.
...
**재고 차감 동시성 제어 (조건부 UPDATE)**:
```sql
-- 1단계: productId 오름차순 정렬 (데드락 방지)
-- 2단계: 각 상품에 대해 조건부 UPDATE
UPDATE products
SET stock_quantity = stock_quantity - :quantity
WHERE id = :productId AND stock_quantity >= :quantity;
-- rowsAffected == 0 → 재고 부족 → throw CONFLICT
```
- 검증(`stock_quantity >= qty`)과 차감이 단일 UPDATE로 원자적으로 실행
- productId 오름차순 정렬로 처리 순서 고정 (데드락 방지)
...
**쿠폰 발급 동시성 제어 (DB UNIQUE + catch)**:
- 수량 제한 없음 — totalQuantity/issuedQuantity 컬럼 미사용
- 중복 발급 방지: `uk_user_coupon_member_template(ref_member_id, ref_coupon_template_id)` in user_coupons
- 선조회로 1차 확인 → INSERT 시도 → UNIQUE 위반 시 `DataIntegrityViolationException` catch
...
| version | INT | NOT NULL, DEFAULT 0 | 조건부 UPDATE 시 version 증가 |
...
**제약**:
- PK: `id`
- UK: `(ref_member_id, ref_coupon_template_id)` — 중복 발급 방지 (최종 방어선)
...
| user_coupons | uk_user_coupon_member_template | (ref_member_id, ref_coupon_template_id) | 중복 발급 방지 (최종 방어선) |
...
```
전략: 조건부 UPDATE (WHERE stock_quantity >= qty)

트랜잭션 내 처리 순서:
1. 상품 목록을 productId 오름차순 정렬 (데드락 방지)
2. 각 상품에 대해:
   a. findByProductId로 상품 조회 (존재 확인 + 스냅샷용)
   b. UPDATE products SET stock_quantity = stock_quantity - :qty
      WHERE id = :productId AND stock_quantity >= :qty;
   c. rowsAffected == 0 → 재고 부족 → 예외 발생 → 트랜잭션 전체 롤백
3. OrderModel + OrderItemModel 저장
4. 트랜잭션 커밋

장점: 락 없이 원자적 차감, 높은 처리량, 단순한 구현
단점: 재고 검증과 차감이 분리되지 않아 로직 파악 시 주의 필요
```
...
### 3. 재고 차감 방식 전환 가능성
- **현재**: 조건부 UPDATE (`WHERE stock_quantity >= qty`) + productId ASC 정렬
- **고트래픽 시 대안**: Redis 분산 락 (DB 부하 분산 필요 시)
